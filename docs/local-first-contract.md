...
capture_diagnostics: null | {elapsed_seconds, warnings, warning_history, route_changes, sources}
```
...
`route_changes` records bounded, name-only default input/output snapshots when
macOS changes the active route during capture; it never changes those devices.
